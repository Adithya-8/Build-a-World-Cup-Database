...
echo "$($PSQL "select name from teams inner join games on teams.team_id=games.winner_id OR teams.team_id = games.opponent_id where round like 'Eighth-Final' and year=2014 order by name")"
